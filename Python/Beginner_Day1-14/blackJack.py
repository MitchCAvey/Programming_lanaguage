...
#a blackjack (0), then the user wins. If the user_score is over 21, then the user loses. If the computer_score is over 21, 
...
def playGame():
    ...
    isGameOver = False
    #print(deal_card())
    #while len(user_cards) <= 1 and len(computer_cards) <= 1:
    for _ in range(2):
...
#print(f"Players Cards: {user_cards} - Sum of cards: ")
#print(f"Players Cards: {computer_cards}")
    
#deal_card()
# calculate_score(user_cards)
#calculate_score([11,11])

    # handCards = list(handOne)
    # print(handCards)
    # if sum(handCards) == 21:
    #     print(f"You have won: {sum(handCards)}")
    #     return 0
    # elif sum(handCards) < 17:
    #     handCards.append(deal_card())
    #     print(handCards)
    #     if sum(handCards) < 17:
    #         handCards.append(deal_card())
    #     elif sum(handCards) > 21:
    #         print(f"You Lose with: {sum(handCards)}")
    #     elif sum(handCards) == 21:
    #         print(f"You have won: {sum(handCards)}")
    #         return 0
    #     elif sum(handCards) < 20 and handCards[2] == 11:
    #         handCards.pop(2)
    #         handCards.insert(2, 1)
    #         print(handCards)

    # for card in handCards:
    #     if card == 11:
    #         print(handCards.index(card))
    #         index = handCards.index(card)
    #         handCards.pop(handCards.index(card))
    #         handCards.insert(index, 1)
    # print(handCards)

            #handCards.insert(handCards.index(card), 1)
        # elif sum(handCards) == 20 and card == 11:
        #     handCards.pop(handCards.index(card))
        # #     handCards.insert(handCards.index(card), 1)
        # #     print(handCards)
        # elif sum(handCards) == 21:
